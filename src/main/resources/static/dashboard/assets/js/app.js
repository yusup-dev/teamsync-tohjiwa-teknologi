...
(function () {
	...
	function setLanguage(lang) {
		if (document.getElementById("header-lang-img")) {
			if (lang == "en") {
				document.getElementById("header-lang-img").src = "/dashboard/assets/images/flags/us.svg";
			} else if (lang == "sp") {
				document.getElementById("header-lang-img").src = "/dashboard/assets/images/flags/spain.svg";
			} else if (lang == "gr") {
				document.getElementById("header-lang-img").src = "/dashboard/assets/images/flags/germany.svg";
			} else if (lang == "it") {
				document.getElementById("header-lang-img").src = "/dashboard/assets/images/flags/italy.svg";
			} else if (lang == "ru") {
				document.getElementById("header-lang-img").src = "/dashboard/assets/images/flags/russia.svg";
			} else if (lang == "ch") {
				document.getElementById("header-lang-img").src = "/dashboard/assets/images/flags/china.svg";
			} else if (lang == "fr") {
				document.getElementById("header-lang-img").src = "/dashboard/assets/images/flags/french.svg";
			} else if (lang == "ar") {
				document.getElementById("header-lang-img").src = "/dashboard/assets/images/flags/ae.svg";
			}
	...
	function renderDynamicHref() {
	    // Mendapatkan semua elemen yang memiliki atribut href
        const elementsWithHref = document.querySelectorAll('[href*="/dashboard/pws/"]');;

        var pwsIdValue = getPwsIdFromPath();

        console.log(pwsIdValue);

        if(pwsIdValue == null) {
            pwsIdValue = getPwsIdFromCookie();
        }

        console.log(pwsIdValue);

        // elementsWithHref adalah NodeList yang berisi semua elemen dengan atribut href
        elementsWithHref.forEach(function(element) {
          // Lakukan sesuatu dengan setiap elemen
          console.log(element);

          var originalString = element.href;
          ...
          const modifiedString = relativePath.replace("pwsId", pwsIdValue);
          element.href = modifiedString;
        });
	}

	function getPwsIdFromPath() {
	    // Mendapatkan current path
        const currentPath = window.location.pathname;

        // Menentukan pola regex untuk mencari pwsId
        const regexPattern = /\/dashboard\/pws\/(\d+)\//;

        // Mencocokkan pola regex dengan current path
        const match = currentPath.match(regexPattern);

        if (match) {
          // Jika ada kecocokan, maka pwsId dapat diambil dari grup regex pertama (indeks 1)
          return pwsId = match[1];
        } else {
          return null;
        }
	}

	function getPwsIdFromCookie() {
    	   return getCookie("pwsId");
    }
					...
					if (elem.querySelectorAll(".notification-item").length > 0) {
						...
						if (!emptyNotificationElem) {
							...
							<div class="w-25 w-sm-50 pt-3 mx-auto">\
								<img src="/dashboard/assets/images/svg/bell.svg" class="img-fluid" alt="user-pic">\
							</div>\
