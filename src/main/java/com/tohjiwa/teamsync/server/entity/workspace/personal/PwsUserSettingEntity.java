...
@Builder
@ToString
@Table(name = "personal_workspace_user_settings")
@Where(clause = "is_deleted = false")
@SQLDelete(sql = "UPDATE personal_workspace_user_settings SET is_deleted = true, deleted_at = now(), status = 'INACTIVE' WHERE id = ?")
public class PwsUserSettingEntity extends DataBaseEntity {
    @Id
    ...
    @JoinColumn(name = "user_id", nullable = false, foreignKey = @ForeignKey(name = "fk_pws_user_settings_users"))
    private UserEntity user;

    @NotNull
    @ToString.Exclude
    @ManyToOne(fetch = FetchType.LAZY, optional = false)
    @JoinColumn(name = "personal_workspace_id", nullable = false, foreignKey = @ForeignKey(name = "fk_pws_user_settings_pws"))
